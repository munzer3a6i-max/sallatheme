...
import initColorScheme from './partials/color-scheme';
import initReveal from './partials/reveal';
import initAnnouncement from './partials/announcement';
...
class App extends AppHelpers {
  ...
  log(message) {
    salla.log(`ThemeApp(Zirar)::${message}`);
    return this;
  }
...
isElementLoaded(selector){
   ...
   * on the way up, so it never covers the content being read. It also steps
   * aside for the sticky add-to-cart bar on the product page.
   */
  initMobileBottomBar() {
    const bar = this.element('#mobile-bottom-bar');
    if (!bar) return;

    // Prefer Twilight's own route resolver; the rendered href stays as fallback.
    bar.querySelectorAll('[data-route]').forEach(link => {
      const resolved = salla.url?.get?.(link.dataset.route);
    ...
    salla.cart.event.onUpdated(summary => {
      const badge = bar.querySelector('[data-cart-count]');
      if (badge) badge.classList.toggle('is-empty', !summary.count);
    });
  }
